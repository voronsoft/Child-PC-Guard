...
--------------------------------------------------------------------------------------
Для запуска проекта:
1 Необходимо создать файл .env на основе файла(.env.example) примера с переменными окружения

2 Строка:
SECRET_KEY=Your_aPPliCAtion's_sECret_code_stRing
Должна содержать любые произвольные знаки без пробелов !!!

3 Строка:
bot_token_telegram=You-need-to-create-a-bot-in-Telegram-and-insert-the-received-token-into-this-line
Должна содержать token бота, который нужно получить в телеграм при создании бота.
Необходим для включения оповещения на вашем телефоне.
Настройка бота производится в основном приложении, вкладка оповещение.
--------------------------------------------------------------------------------------
