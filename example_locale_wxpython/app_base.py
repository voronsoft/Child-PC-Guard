...
class BaseApp(wx.App, InspectionMixin):
    def OnInit(self):
        ...
        self.locale = None
        # Добавляет путь, где будут храниться файлы с переводами. Обычно это папка locale в корне проекта,
        # где хранятся .mo файлы с переводами.
        wx.Locale.AddCatalogLookupPathPrefix('locale')

        # В этом методе происходит обновление языка на выбранный
        self.updateLanguage(self.appConfig.Read(u"Language"))

        return True

    def doConfig(self):
        """Настройте файл конфигурации приложения"""
        # папка конфигурации
        ...
        if not self.appConfig.HasEntry(u'Language'):
            # при первом запуске мы по умолчанию устанавливаем немецкий язык
    ...
    def updateLanguage(self, lang):
        ...
        # создать объект локали для этого языка
